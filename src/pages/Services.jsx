        ...
        return (
          <div key={option} className="mb-2">
            <button
              ...
              className="w-full text-left text-2xl font-bold text-theme py-2 border-b-2 border-white/10 flex justify-between items-center"
            >
              <span>{caseStudyData[option].name}</span>
              <motion.span
              ...
              <ul className="mt-8 flex justify-center flex-col gap-8">
                <AnimatePresence>
                  {Object.keys(caseStudyData).map((option) => (
                    <motion.h1
                      key={option}
                      className={`text-5xl lg:text-6xl capitalize transition-all duration-300 font-bold cursor-pointer ${
                        selected === option
